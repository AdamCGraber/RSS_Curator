...
def ingest(payload: IngestRequest | None = None, db: Session = Depends(get_db)):
    ...
    if payload and (payload.start_date is not None or payload.end_date is not None):
        ...
        # No explicit date range provided: preserve rolling runtime semantics.
        ...
        if (
            payload is None
            and prefs.cluster_time_window_start is not None
            and prefs.cluster_time_window_end is not None
        ):
